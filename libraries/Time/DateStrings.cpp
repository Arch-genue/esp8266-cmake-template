...
{
    const char* ptr = monthNames_P[month];  // напрямую обращаемся, без pgm_read_ptr
    strcpy(buffer, ptr);
    return buffer;
}
// char* monthStr(uint8_t month)
// {
//    // const char* ptr = reinterpret_cast<const char*>(pgm_read_ptr(&monthNames_P[month]));
//    const char* ptr = (const char*)(uintptr_t)pgm_read_ptr(&monthNames_P[month]);

//    strcpy_P(buffer, ptr);
//    // strcpy_P(buffer, (const char*)pgm_read_ptr(&(monthNames_P[month])));
...
{
    const char* ptr = dayNames_P[day];  // напрямую обращаемся, без pgm_read_ptr
    strcpy(buffer, ptr);
    return buffer;
}
...
// char* dayStr(uint8_t day) 
// {
//    strcpy_P(buffer, (const char*)pgm_read_ptr(&(dayNames_P[day])));
