 ...
 * 27        4        ограничение мощности лампы CURRENT_LIMIT
...
#define EEPROM_CURRENT_LIMIT_ADDRESS         (27U)         // адрес в EEPROM памяти для записи лимита мощности
...
{
  public:
    static void InitEepromSettings(ModeType modes[], AlarmType alarms[], uint8_t* espMode, bool* onFlag, uint8_t* dawnMode, uint8_t* currentMode, bool* buttonEnabled, uint32_t* currentLimit,
      void (*readFavoritesSettings)(), void (*saveFavoritesSettings)(), void (*restoreDefaultSettings)())
    {
      ...
      if (EEPROM.read(EEPROM_FIRST_RUN_ADDRESS) != EEPROM_FIRST_RUN_MARK)
      {
        ...
        WriteUint32(EEPROM_CURRENT_LIMIT_ADDRESS, 1000);
      ...
      *buttonEnabled = EEPROM.read(EEPROM_ESP_BUTTON_ENABLED_ADDRESS);
      //int curlim = ReadUint32(EEPROM_CURRENT_LIMIT_ADDRESS);
      //Serial.println(curlim);
      *currentLimit = ReadUint32(EEPROM_CURRENT_LIMIT_ADDRESS);
    }
    ...
    static void saveCurrentLimit(uint32_t* currentLimit)
    {
      WriteUint32(EEPROM_CURRENT_LIMIT_ADDRESS, *currentLimit);
      EEPROM.commit();
    }
