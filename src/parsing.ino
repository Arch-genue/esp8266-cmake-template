...
void processInputBuffer(char *inputBuffer, char *outputBuffer, bool generateOutput)
{
    ...
    else if (!strncmp_P(inputBuffer, PSTR("LIST"), 4)) // передача списка эффектов по запросу от приложения (если поддерживается приложением)
    {
      ...
      #if defined(USE_SECRET_COMMANDS) || defined(USE_MANUAL_TIME_SETTING) // вкорячиваем ручную синхронизацию времени пока что сюда. пока нет другой функции в приложении...
...
*/        
          ...
          /*
           * pwlimit=3000 from 0 to 9999
           * 
           */
          else if (!strncmp_P(inputBuffer, PSTR("TXT-pwlimit="), 12) && (BUFF.length() > 13)){
            //Serial.println(BUFF); //8, 12
            //Serial.println(BUFF.substring(12, 16).toInt());
            int pwlim = BUFF.substring(12, 17).toInt();
            if ( pwlim < 10000 ) {
              currentLimit = pwlim;
              EepromManager::saveCurrentLimit(&currentLimit);
              showWarning(CRGB::Blue, 2000U, 500U);        // мигание оранжевым цветом 2 секунды
              delay(500);
              ESP.restart();
            } else showWarning(CRGB::Red, 2000U, 500U);      // мигание красным цветом 2 секунды (ошибка)
          }
